...
const agentsMd = readFile('AGENTS.md') || readFile('AGENTS.md');
if (agentsMd) {
  const goal = extractSection(agentsMd, 'Current Goal');
  if (goal && !output.goal) output.goal = goal.split('\n')[0].trim();

  const doNot = extractSection(agentsMd, 'Do Not Do');
  if (doNot) {
  ...
  const stack = extractSection(agentsMd, 'Tech Stack');
  if (stack && output.stack.length === 0) {
    output.stack = stack.split(/[,\n]/).map(s => s.replace(/^[-*]\s+/, '').trim()).filter(Boolean);
  }

  // Description from first section or "What This Is"
  const whatItIs = extractSection(agentsMd, 'What This Is') || extractSection(agentsMd, 'About');
