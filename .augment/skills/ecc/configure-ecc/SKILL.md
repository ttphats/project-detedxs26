---
name: configure-ecc
description: Interactive installer for Everything Augment Agent — guides users through selecting and installing skills and rules to user-level or project-level directories, verifies paths, and optionally optimizes installed files.
origin: ECC
---

# Configure Everything Augment Agent (ECC)

An interactive, step-by-step installation wizard for the Everything Augment Agent project. Uses `AskUserQuestion` to guide users through selective installation of skills and rules, then verifies correctness and offers optimization.
...
This skill must be accessible to Augment Agent before activation. Two ways to bootstrap:
1. **Via Plugin**: `/plugin install everything-claude-code` — the plugin loads this skill automatically
2. **Manual**: Copy only this skill to `~/.augment/skills/ecc/configure-ecc/SKILL.md`, then activate by saying "configure ecc"
...
Options:
  - "User-level (~/.augment/)" — "Applies to all your Augment Agent projects"
  - "Project-level (.augment/)" — "Applies only to the current project"
  - "Both" — "Common/shared items user-level, project-specific items project-level"
```

Store the choice as `INSTALL_LEVEL`. Set the target directory:
- User-level: `TARGET=~/.augment`
- Project-level: `TARGET=.augment` (relative to current project root)
- Both: `TARGET_USER=~/.augment`, `TARGET_PROJECT=.augment`
...
```bash
grep -rn "~/.augment/" $TARGET/skills/ $TARGET/rules/
grep -rn "../common/" $TARGET/rules/
grep -rn "skills/" $TARGET/skills/
```

**For project-level installs**, flag any references to `~/.augment/` paths:
- If a skill references `~/.augment/settings.json` — this is usually fine (settings are always user-level)
- If a skill references `~/.augment/skills/` or `~/.augment/rules/` — this may be broken if installed only at project level
...
- `continuous-learning-v2` references `~/.augment/homunculus/` directory
...
3. **Issue**: What's wrong (e.g., "references ~/.augment/skills/python-patterns but python-patterns was not installed")
4. **Suggested fix**: What to do (e.g., "install python-patterns skill" or "update path to .augment/skills/")
...
### "Skills not being picked up by Augment Agent"
- Verify the skill directory contains a `SKILL.md` file (not just loose .md files)
- For user-level: check `~/.augment/skills/<skill-name>/SKILL.md` exists
- For project-level: check `.augment/skills/<skill-name>/SKILL.md` exists
...
- Restart Augment Agent after installing rules

### "Path reference errors after project-level install"
- Some skills assume `~/.augment/` paths. Run Step 4 verification to find and fix these.
- For `continuous-learning-v2`, the `~/.augment/homunculus/` directory is always user-level — this is expected and not an error.
