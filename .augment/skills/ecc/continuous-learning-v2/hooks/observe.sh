...
# Augment Agent passes hook data via stdin as JSON.
#
...
# Adapted for Augment Agent (no hook support — run manually).
...
# Read JSON from stdin (Augment Agent hook format)
INPUT_JSON=$(cat)
...
if [ -n "$STDIN_CWD" ] && [ -d "$STDIN_CWD" ]; then
  ...
  export AUGMENT_PROJECT_DIR="${_GIT_ROOT:-$STDIN_CWD}"
fi
...
CONFIG_DIR="${HOME}/.augment/homunculus"
...
_ECC_SKIP_PATHS="${ECC_OBSERVE_SKIP_PATHS:-observer-sessions,.augment-mem}"
if [ -n "$STDIN_CWD" ]; then
...
# Pass HOOK_PHASE via env var since Augment Agent does not include hook type in stdin JSON
...
try:
    ...
    # Augment Agent does NOT include a "hook_type" field in the stdin JSON,
    # so we rely on the shell argument ("pre" or "post") instead.
    hook_phase = os.environ.get("HOOK_PHASE", "post")
    event = "tool_start" if hook_phase == "pre" else "tool_complete"

    # Extract fields - Augment Agent hook format
