...
Integration reference for Plankton (credit: @alxfazio), a write-time code quality enforcement system for Augment Agent. Plankton runs formatters and linters on every file edit via PostToolUse hooks, then spawns Claude subprocesses to fix violations the agent didn't catch.
...
Every time Augment Agent edits or writes a file, Plankton's `multi_linter.sh` PostToolUse hook runs:
...
# Start Augment Agent — hooks activate automatically
claude
```

No install command, no plugin config. The hooks in `.augment/settings.json` are picked up automatically when you run Augment Agent in the Plankton directory.

### Per-Project Integration

To use Plankton hooks in your own project:

1. Copy `.augment/hooks/` directory to your project
2. Copy `.augment/settings.json` hook configuration
...
Plankton's `.augment/hooks/config.json` controls all behavior:
