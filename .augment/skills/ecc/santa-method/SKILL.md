...
```python
# Spawn reviewers in parallel (Augment Agent subagents)
...
### Pattern A: Augment Agent Subagents (Recommended)

Subagents provide true context isolation. Each reviewer is a separate process with no shared state.

```bash
# In a Augment Agent session, use the Agent tool to spawn reviewers
