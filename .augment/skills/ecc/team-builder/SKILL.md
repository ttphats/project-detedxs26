...
2. **File glob** (fallback, for reading agent content) — agent markdown files are read from:
   ...
   - `~/.augment/agents/**/*.md` + `~/.augment/agents/*.md` — global user agents
...
- **User agents** have no prefix. Read the corresponding markdown file from `~/.augment/agents/` or `./agents/` to extract the name and description.
...
- **Parallel Agent calls, not TeamCreate.** This skill uses parallel Agent tool calls for independent work. TeamCreate (a Augment Agent tool for multi-agent dialogue) is only needed when agents must debate or respond to each other.
