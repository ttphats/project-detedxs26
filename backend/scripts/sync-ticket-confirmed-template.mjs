/**
 * Upsert admin Email Template TICKET_CONFIRMED with multi-ticket {{ticketUnitsHtml}}.
 */
...
<html lang="vi"><head><meta charset="utf-8"/><meta name="viewport" content="width=device-width,initial-scale=1"/><title>Vé điện tử - {{eventName}}</title></head>
<body style="margin:0;padding:0;background:#f4f4f4;font-family:Helvetica,Arial,sans-serif;">
<table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" style="background:#f4f4f4;"><tr><td align="center" style="padding:32px 16px;">
<table role="presentation" width="600" cellspacing="0" cellpadding="0" border="0" style="max-width:600px;width:100%;">
<tr><td style="text-align:center;padding-bottom:24px;"><div style="display:inline-block;background:#ea251a;padding:8px 20px;"><span style="font-size:10px;font-weight:900;color:#fff;letter-spacing:3px;">✓ THANH TOÁN THÀNH CÔNG</span></div></td></tr>
<tr><td style="text-align:center;padding-bottom:20px;">
<h2 style="margin:0 0 8px;font-size:28px;font-weight:900;color:#000;">TED<span style="font-weight:300;">x</span></h2>
<p style="margin:0;font-size:11px;letter-spacing:2px;color:#666;text-transform:uppercase;">FPT University HCMC</p>
<h1 style="margin:16px 0 8px;font-size:24px;font-weight:900;color:#000;">{{eventName}}</h1>
...
<tr><td style="padding:24px 0 8px;font-size:12px;font-weight:800;color:#000;letter-spacing:1px;text-transform:uppercase;">Thông tin đơn</td></tr>
...
<tr><td style="padding:28px 0 8px;font-size:12px;font-weight:800;color:#000;letter-spacing:1px;text-transform:uppercase;">Danh sách vé · mỗi QR = 1 check-in</td></tr>
<tr><td>{{ticketUnitsHtml}}</td></tr>
<tr><td style="background:#fff;border-radius:12px;padding:16px 20px;">
...
const payload = {
  ...
  purpose,
  category: 'ORDER',
  description: 'Vé PAID multi QR — chỉ render template admin. Dùng {{ticketUnitsHtml}}.',
  subject: SUBJECT,
  htmlContent: HTML,
  textContent: 'TEDx - Don {{orderNumber}} - {{customerName}} - {{eventName}} - {{ticketUrl}}',
  variables: VARIABLES,
  isActive: true,
  isDefault: true,
}
