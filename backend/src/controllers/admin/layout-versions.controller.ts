...
/**
 * PUT /api/admin/layout-versions/:id
 */
export async function update(request: FastifyRequest, reply: FastifyReply) {
  try {
    requireAdmin(request.user!)
    const {id} = request.params as {id: string}
    const {version_name, description, layout_config, seats_data} = request.body as {
      version_name?: string
      description?: string
      layout_config?: any
      seats_data?: any
    ...
    await layoutVersionsService.updateLayoutVersion(id, {
      version_name,
      description,
      layout_config,
      seats_data,
    ...
    return reply.status(500).send({
      success: false,
      error: error.message || 'Failed to update layout version',
    })
  }
}

/**
 * POST /api/admin/layout-versions/:id/publish
 */
export async function publish(request: FastifyRequest, reply: FastifyReply) {
  try {
    ...
    return reply.status(500).send({
      success: false,
      error: error.message || 'Failed to publish layout version',
    })
  }
}
