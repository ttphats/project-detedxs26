...
description: "Patterns and architectures for autonomous Augment Agent loops — from simple sequential pipelines to RFC-driven multi-agent DAG systems."
origin: ECC
---
...
Patterns, architectures, and reference implementations for running Augment Agent autonomously in loops. Covers everything from simple `claude -p` pipelines to full RFC-driven multi-agent DAG orchestration.
...
The `claude -p` flag runs Augment Agent non-interactively with a prompt, exits when done. Chain calls to build a pipeline:
...
echo "Focus areas: auth module, API rate limiting" > .augment-context.md
augment agent -p "Read .augment-context.md for priorities. Work through them in order."
rm .augment-context.md
```
...
1. Loads conversation history from `~/.augment/claw/{session}.md`
...
### Implementation via Augment Agent Commands

Create `.augment/commands/infinite.md`:
...
**A production-grade shell script** that runs Augment Agent in a continuous loop, creating PRs, waiting for CI, and merging automatically. Created by AnandChowdhary (credit: @AnandChowdhary).
