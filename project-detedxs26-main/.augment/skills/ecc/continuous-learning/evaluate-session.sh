...
# Runs on Stop hook to extract reusable patterns from Augment Agent sessions
#
# Why Stop hook instead of UserPromptSubmit:
# - Stop runs once at session end (lightweight)
# - UserPromptSubmit runs every message (heavy, adds latency)
#
# Adapted for Augment Agent (no hook support — run manually at session end)
#
# Patterns to detect: error_resolution, debugging_techniques, workarounds, project_specific
# Patterns to ignore: simple_typos, one_time_fixes, external_api_issues
# Extracted skills saved to: ~/.augment/skills/learned/

set -e

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
CONFIG_FILE="$SCRIPT_DIR/config.json"
LEARNED_SKILLS_PATH="${HOME}/.augment/skills/learned"
...
if [ -f "$CONFIG_FILE" ]; then
  if ! command -v jq &>/dev/null; then
    ...
    LEARNED_SKILLS_PATH=$(jq -r '.learned_skills_path // "~/.augment/skills/learned/"' "$CONFIG_FILE" | sed "s|~|$HOME|")
  fi
fi
...
# Get transcript path from stdin JSON (Augment Agent hook input)
