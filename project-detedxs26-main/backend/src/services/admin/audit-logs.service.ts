 ...
 */
export async function listAuditLogs(input: ListAuditLogsInput, user: JWTPayload) {
  ...
  return {
    ...
    stats: {
      byAction: actionStats.map((s: any) => ({action: s.action, count: s._count.action})),
      byEntity: entityStats.map((s: any) => ({entity: s.entity, count: s._count.entity})),
    },
  }
}
