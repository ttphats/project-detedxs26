        ...
        if (activeVersion) {
          ...
          try {
            ...
            if (seatsData.success && seatsData.data.seats?.length > 0) {
              ...
              console.log(
                '🎬 No seats in database - use "Tạo ghế" button to create layout',
              );
        ...
        <Card title="Cấu hình Layout" size="small">
          {!selectedEvent && (
            <div className="mb-4 p-3 bg-yellow-50 border border-yellow-200 rounded-lg">
              <span className="text-yellow-800">
                ⚠️ Vui lòng chọn sự kiện trước khi tạo layout
              </span>
            </div>
          )}
            ...
            <div className="flex items-end">
              <Button
                ...
                onClick={() => {
                  if (!currentType) {
                    message.warning(
                      "⚠️ Vui lòng chọn loại vé bên dưới trước khi tạo ghế!",
                    );
                    return;
                  }
                  ...
                  const newSeats = generateSeatsFromConfig(
                    config,
                    currentType,
                    ticketTypes,
                  );
                  setSeats(newSeats);
                  setHasChanges(true);
                  const ticketInfo = getTicketTypeFromSeat(
                    currentType,
                    ticketTypes,
                  );
                  message.success(
          ...
          <div className="flex flex-wrap gap-4 mt-4 pt-4 border-t">
            <span className="text-sm text-gray-500">
              Tổng ghế:{" "}
              <strong className="text-blue-600">{seats.length}</strong>
            </span>
          ...
          <div className="flex flex-wrap gap-3 items-center">
            ...
            {Array.from(
              ...
              <Button
                key={tt.id}
                type={
                  currentType === `LEVEL_${tt.level}` ? "primary" : "default"
                }
                style={
              ...
              {uniqueRows.length === 0 ? (
                <div className="text-center text-gray-300 py-20">
                  ...
                  <p className="text-sm text-gray-500">
                    ...
                    <strong className="text-blue-400">"Tạo ghế"</strong> để bắt
                    đầu
                  </p>
                </div>
              ) : (
