...
function getTimeRemaining(expiresAt: string | Date | null) {
  ...
  if (diffMins < 5)
    return { expired: false, text: `${diffMins} phút`, color: "red" };
  if (diffMins < 10)
    return { expired: false, text: `${diffMins} phút`, color: "orange" };
...
interface Order {
  ...
  // Seat management hidden — ticket inventory is source of truth
  seats?: Array<{
    seatNumber: string;
    seatType: string;
    section: string;
...
export default function OrdersPage() {
  ...
  const handleReject = async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        const emailError = data.data?.emailError;
        const released =
          data.data?.releasedSeats ?? data.data?.releasedTickets ?? 0;
        if (emailStatus === "SENT") {
          message.success(
            `Đã từ chối đơn hàng. Đã hoàn ${released} vé vào kho. Email thông báo đã gửi.`,
          );
        } else {
          message.warning({
            content: `Đã từ chối đơn hàng (hoàn ${released} vé) nhưng gửi email thất bại: ${emailError || "Unknown error"}.`,
            duration: 8,
          });
        }
        setRejectModal(null);
    ...
    Modal.confirm({
      ...
      content: (
        <div>
          ...
          <ul className="mt-2 text-sm text-gray-600">
            ...
            <li>• Vé / inventory sẽ được hoàn về kho</li>
      ...
      onOk: async () => {
        ...
        try {
          ...
          if (data.success) {
            message.success(
              `Đã xóa đơn hàng ${record.orderNumber}. Đã hoàn ${data.data?.releasedSeats || 0} vé vào kho.`,
            );
            fetchData();
          } else {
  ...
  const pendingSelectedOrdersWithEmail = pendingSelectedOrders.filter(
    (o) => o.customerEmail,
  );
    ...
    Modal.confirm({
      ...
      onOk: async () => {
        ...
        if (totalReleasedSeats > 0)
          parts.push(`${totalReleasedSeats} vé đã hoàn kho`);
        if (failCount > 0) parts.push(`${failCount} thất bại`);
  ...
  const columns: ColumnsType<Order> = [
    ...
    },
    {
      ...
      width: 200,
      render: (_, record) => {
        const lines = record.ticketLines || [];
        if (!lines.length) {
          return (
            <span className="text-gray-400 text-xs">
              {record.ticketCount ? `${record.ticketCount} vé` : "—"}
            </span>
          );
        }
        return (
          <div className="flex flex-wrap gap-1">
            {lines.slice(0, 4).map((l, i) => (
              <Tag
                key={i}
                color={
                  l.name.toUpperCase().includes("VIP")
                    ? "red"
                    : l.name.toUpperCase().includes("DONOR")
                      ? "gold"
                      : l.name.toUpperCase().includes("EARLY")
                        ? "green"
                        : "blue"
                }
              >
            ...
            {lines.length > 4 && <Tag>+{lines.length - 4}</Tag>}
          </div>
        );
      },
    },
            ...
            <Button
              ...
              onClick={() => {
                const exportData = orders.map((order) => ({
                  ...
                  seats: order.ticketSummary || "",
                  seatTypes: (order.ticketLines || [])
                    .map((l) => l.name)
                    .join(", "),
                  tickets: order.ticketSummary || "",
              ...
              <span className="font-medium">
                ...
                {pendingSelectedOrdersWithEmail.length > 0 && (
                  <span className="text-orange-600 ml-2">
                    ({pendingSelectedOrdersWithEmail.length} chờ thanh toán có
                    email)
                  </span>
                )}
              </span>
            ...
            <div className="space-y-4">
              <Descriptions bordered column={2} size="small">
                ...
                <Descriptions.Item label="Tổng tiền" span={2}>
                  <strong className="text-lg text-green-600">
                    {Math.round(
                      Number(detailModal.totalAmount || 0),
              ...
              <div>
                <h4 className="font-medium mb-2">
                  Vé (
                  {detailModal.ticketCount || detailModal.tickets?.length || 0})
                </h4>
                {(detailModal.ticketLines || []).length > 0 && (
                  <div className="flex flex-wrap gap-1 mb-3">
                    {detailModal.ticketLines!.map((l, i) => (
                      <Tag key={i} color="blue">
                        {l.name} ×{l.quantity} —{" "}
                        {Math.round(l.lineTotal).toLocaleString("vi-VN")} ₫
                      </Tag>
                    ))}
                  </div>
                )}
                <Table
                  ...
                  columns={[
                    ...
                    },
                    {
                      ...
                      key: "checkedIn",
                      render: (v: boolean) =>
                        v ? <Tag color="success">Đã vào</Tag> : <Tag>Chưa</Tag>,
                    },
                    {
                      title: "Giá",
                      dataIndex: "price",
          ...
          <div className="space-y-4">
            <div className="bg-green-50 border border-green-200 rounded-lg p-4">
              ...
              <div className="text-sm text-green-700 space-y-1">
                ...
                <p>
                  ...
                  {Math.round(
                    Number(confirmModal?.totalAmount || 0),
                  ).toLocaleString("vi-VN")}{" "}
                  ₫
                </p>
                <p>
                  <strong>Vé:</strong>{" "}
                  {confirmModal?.ticketSummary ||
                    (confirmModal?.ticketCount
                      ? `${confirmModal.ticketCount} vé`
                      : "—")}
                </p>
              </div>
            </div>
            ...
            <div className="bg-blue-50 border border-blue-200 rounded-lg p-3">
              ...
              <ul className="text-blue-700 text-sm mt-1 list-disc list-inside">
                ...
                <li>Tạo mã TKT + QR riêng cho từng vé</li>
                <li>Gửi email vé (template admin) kèm danh sách QR</li>
              </ul>
            </div>
        ...
        >
          <div className="space-y-4">
            ...
            <p className="text-gray-500 text-sm">
              Vé sẽ được hoàn về kho và email từ chối sẽ được gửi cho khách
              hàng.
            </p>
            <Input.TextArea
