...
export default function OrdersPage() {
  ...
  const handleReject = async () => {
    ...
    try {
      ...
      const data = await res.json()
      if (data.success) {
        message.success(`Đã từ chối đơn hàng. ${data.data.releasedSeats} ghế đã được mở lại.`)
        setRejectModal(null)
        setRejectReason('')
  ...
  const columns: ColumnsType<Order> = [
    ...
    },
    {
      ...
      align: 'right',
      render: (amount) => (
        <span className='font-medium text-green-600'>{amount.toLocaleString('vi-VN')} ₫</span>
      ),
    },
    {
      title: 'Trạng thái',
              ...
              <Popconfirm
                ...
                description={
                  record.status === 'PENDING_CONFIRMATION'
                    ? 'Khách hàng đã gửi thông tin thanh toán. Xác nhận và gửi vé?'
                    : 'Bạn đã nhận được chuyển khoản?'
                }
            ...
            <div className='space-y-4'>
              <Descriptions bordered column={2} size='small'>
                ...
                <Descriptions.Item label='Ngày thanh toán'>
                  {detailModal.paidAt ? new Date(detailModal.paidAt).toLocaleString('vi-VN') : '-'}
                </Descriptions.Item>
