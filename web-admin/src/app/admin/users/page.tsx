...
export default function UsersPage() {
  ...
  const canManageUser = (targetRole: string): boolean => {
    ...
    // ADMIN can only manage STAFF and USER
  ...
  const handleCreate = () => {
    setEditingUser(null)
    setInitialFormValues({isActive: true})
    setModalOpen(true)
  }
  ...
  const stats = {
    ...
    admins: users.filter((u) => ['SUPER_ADMIN', 'ADMIN'].includes(u.roleName)).length,
  }
    ...
    <AdminLayout>
      <div className='space-y-6'>
        ...
        <Row gutter={16}>
          <Col span={8}>
            <Card>
              <Statistic title='Tổng Users' value={pagination.total} prefix={<TeamOutlined />} />
            </Card>
          </Col>
          <Col span={8}>
            <Card>
          ...
          >
            ...
            >
              <Select placeholder='Chọn vai trò'>
                {(roles || [])
                  .filter((role) => canManageUser(role.name))
                  .map((role) => (
                    <Select.Option key={role.id} value={role.id}>
                      <Tag color={roleColors[role.name] || 'default'}>{role.name}</Tag>
                      {role.description && (
