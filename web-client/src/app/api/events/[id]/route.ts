...
export async function GET(request: NextRequest, {params}: {params: Promise<{id: string}>}) {
  try {
    ...
    const event = await query<Event>(`SELECT * FROM events WHERE id = ? OR slug = ? LIMIT 1`, [
      id,
      id,
    ])
    ...
    // Get ticket types for this event (handle error if table doesn't exist)
    let ticketTypes: TicketType[] = []
    try {
      ticketTypes = await query<TicketType>(
        `SELECT * FROM ticket_types WHERE event_id = ? AND is_active = 1 ORDER BY sort_order, name`,
        [eventData.id]
      )
    } catch (error: any) {
      console.error('Failed to fetch ticket types:', error)
      // Continue without ticket types if table doesn't exist
    }
