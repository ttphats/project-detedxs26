...
export async function GET(request: NextRequest, {params}: {params: Promise<{id: string}>}) {
  try {
    ...
    // Debug logging
    const lockedSeatsCount = seatMap
      .flatMap((r) => r.seats)
      .filter((s) => s.status === 'locked' || s.status === 'locked_by_me').length
    if (lockedSeatsCount > 0) {
      console.log(
        `[GET SEATS] Event ${eventId}, Session ${sessionId?.substring(
          0,
          15
        )}..., Locked seats: ${lockedSeatsCount}, Total locks in DB: ${locks.length}`
      )
    }
