...
export async function GET(request: NextRequest) {
  try {
    ...
    const locks = await query<SeatLock & { seat_number: string; row: string; col: number; section: string; seat_type: string; price: number }>(
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (existingLocks.length > 0) {
      ...
      return NextResponse.json({
        success: false,
        error: `Seats ${lockedSeats.map((s) => s.seat_number).join(', ')} are being selected by another customer`,
