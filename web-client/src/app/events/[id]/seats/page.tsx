...
export default function SeatSelectionPage({params}: {params: Promise<{id: string}>}) {
  ...
  useEffect(() => {
    ...
    const pollInterval = setInterval(async () => {
      try {
        const res = await fetch(`/api/events/${id}/seats?sessionId=${sessionId}`)
        const data = await res.json()
        if (data.success && data.data.seatMap) {
          setEvent((prev) => (prev ? {...prev, seatMap: data.data.seatMap} : prev))
        }
      } catch (err) {
                  ...
                  <div className='min-w-[600px] sm:min-w-[500px] space-y-3 sm:space-y-3 px-2'>
                    {(event.seatMap || []).map((row) => {
                      ...
                      if (hasDefinedSections) {
                        leftSeats = sortedSeats.filter((s) => s.section === 'LEFT')
                        rightSeats = sortedSeats.filter((s) => s.section === 'RIGHT')
                      } else {
                              ...
                              {leftSeats.map((seat) => {
                                ...
                                return (
                                  <Seat
                                    ...
                                    status={finalStatus}
                                    price={seat.price}
                                    seatType={seat.seatType as 'VIP' | 'STANDARD' | 'ECONOMY'}
