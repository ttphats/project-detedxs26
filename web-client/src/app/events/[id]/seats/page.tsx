...
export default function SeatSelectionPage({
    ...
    const fetchEventAndRestoreLocks = async () => {
      try {
        ...
        const isNavigatingToCheckout =
          sessionStorage.getItem("navigating_to_checkout") === "true";
        ...
        if (!isNavigatingToCheckout) {
          console.log(
            "[SEAT SELECTION] User returned to seat selection, clearing old locks",
          );
  ...
  useEffect(() => {
    ...
    const eventSource = new EventSource(
      `/api/events/${id}/seats/stream?sessionId=${sessionId}`,
    );
    ...
    eventSource.onmessage = (event) => {
      try {
        ...
        if (message.type === "INITIAL") {
          ...
          setEvent((prev) =>
            prev ? { ...prev, seatMap: message.data.seatMap } : prev,
          );
        } else if (message.type === "UPDATE") {
          // Real-time update
          console.log(
            "[SSE] Received seat update:",
            message.data.eventData?.type,
          );
          setEvent((prev) =>
            prev ? { ...prev, seatMap: message.data.seatMap } : prev,
          );
  ...
  useEffect(() => {
    ...
    const handleBeforeUnload = () => {
      ...
      const navigatingToCheckout = sessionStorage.getItem(
        "navigating_to_checkout",
      );
      if (navigatingToCheckout === "true") {
        sessionStorage.removeItem("navigating_to_checkout");
        console.log(
          "[CLEANUP] Skipping lock deletion - navigating to checkout",
        );
        return;
      }
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-xs text-gray-400">
                    {selectedSeats.length > 0
                      ? `${
                          selectedSeats.filter((s) => s.seatType === "VIP")
                            .length > 0
                            ? "VIP"
                            : ""
                        }${
