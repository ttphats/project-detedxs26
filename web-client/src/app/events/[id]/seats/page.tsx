...
export default function SeatSelectionPage({params}: {params: Promise<{id: string}>}) {
  ...
  useEffect(() => {
    ...
    const fetchEventAndRestoreLocks = async () => {
      try {
        ...
        // Check if user is navigating back from checkout
        const isNavigatingToCheckout = sessionStorage.getItem('navigating_to_checkout') === 'true'

        // If NOT navigating to checkout, clear the flag and old locks
        // This handles the case when user navigates BACK to seat selection
        if (!isNavigatingToCheckout) {
          console.log('[SEAT SELECTION] User returned to seat selection, clearing old locks')

          // Clear old locks from database
          try {
            ...
            console.log('[SEAT SELECTION] Old locks cleared')
          } catch (err) {
            console.error('[SEAT SELECTION] Failed to clear old locks:', err)
          }
        ...
        // Only restore locks if we're coming from checkout (preserving user's selection)
        if (isNavigatingToCheckout) {
          // Restore locks from this session (for tab sync)
          const locksRes = await fetch(`/api/seats/lock?sessionId=${sessionId}&eventId=${id}`)
  ...
  // Save selectedSeats to sessionStorage for beforeunload cleanup
  useEffect(() => {
    if (typeof window === 'undefined') return
  ...
  // SSE connection for real-time seat updates (replaces polling)
  useEffect(() => {
    if (!id || !sessionId || loading) return

    console.log('[SSE] Connecting to seat updates stream...')

    // Create EventSource for SSE
    const eventSource = new EventSource(`/api/events/${id}/seats/stream?sessionId=${sessionId}`)
    eventSourceRef.current = eventSource

    eventSource.onopen = () => {
      console.log('[SSE] Connected to seat updates stream')
    }
    ...
    eventSource.onmessage = (event) => {
      try {
        ...
        if (message.type === 'INITIAL') {
          // Initial seat map
          console.log('[SSE] Received initial seat map')
          setEvent((prev) => (prev ? {...prev, seatMap: message.data.seatMap} : prev))
        } else if (message.type === 'UPDATE') {
          // Real-time update
          console.log('[SSE] Received seat update:', message.data.eventData?.type)
          setEvent((prev) => (prev ? {...prev, seatMap: message.data.seatMap} : prev))
    ...
    // Cleanup on unmount
    return () => {
      console.log('[SSE] Disconnecting from seat updates stream')
      eventSource.close()
  ...
  // Cleanup locks on page unload (only runs once on mount)
  useEffect(() => {
    if (!sessionId) return

    const handleBeforeUnload = () => {
      // Don't delete locks if navigating to checkout
      const navigatingToCheckout = sessionStorage.getItem('navigating_to_checkout')
      if (navigatingToCheckout === 'true') {
        sessionStorage.removeItem('navigating_to_checkout')
        console.log('[CLEANUP] Skipping lock deletion - navigating to checkout')
        return
      }

      // Get current selected seats from sessionStorage (more reliable than closure)
      const currentSessionId = sessionStorage.getItem('seat_session_id')
      const storedSeats = sessionStorage.getItem(`selected_seats_${id}`)

      if (storedSeats && currentSessionId) {
        try {
          const seats = JSON.parse(storedSeats)
          if (seats.length > 0) {
            console.log('[CLEANUP] Deleting locks on page unload')
    ...
    // Cleanup on component unmount (not on every state change)
    return () => {
  ...
  const handleCheckout = async () => {
    ...
    try {
      ...
      if (!extendData.success) {
        throw new Error(extendData.error || 'Failed to extend lock')
      }
      ...
      console.log('[CHECKOUT] Created pending order:', orderData.data.orderNumber)
      ...
      window.location.href = `/checkout?event=${id}&order=${orderData.data.orderNumber}&token=${orderData.data.accessToken}`
    } catch (error) {
              ...
              <div className='glass-panel rounded-xl sm:rounded-2xl p-4 sm:p-6 md:p-8 animate-fade-in relative overflow-hidden'>
                ...
                <div className='flex flex-wrap justify-center gap-3 sm:gap-6 md:gap-10 mb-4 sm:mb-8 text-xs sm:text-sm'>
                  <div className='flex items-center gap-2 sm:gap-3 px-3 sm:px-4 py-1.5 sm:py-2 rounded-full bg-gradient-to-r from-yellow-500/10 to-orange-500/10 border border-yellow-500/30'>
                    ...
                    <span className='text-yellow-400 font-semibold'>VIP (A-B)</span>
                  </div>
                ...
                <div className='overflow-x-auto mobile-hide-scrollbar pb-4'>
                  ...
                  <div className='min-w-[600px] sm:min-w-[500px] space-y-3 sm:space-y-3 px-2'>
                    {event.seatMap.map((row) => {
                      ...
                      const hasVIPSeats = row.seats.some((s) => s.seatType === 'VIP')
                      ...
                      return (
                        ...
                        >
                          <span
                            className={`w-6 sm:w-8 text-center font-bold text-sm sm:text-base ${
                              hasVIPSeats ? 'text-orange-400' : 'text-gray-500'
                            }`}
                          >
                            {row.row}
                          </span>
                          <div className='flex gap-1 sm:gap-2 flex-1 justify-center items-center'>
                            ...
                            <div className='flex gap-1 sm:gap-2'>
                              {leftSeats.map((seat) => {
                                // Determine final status: prioritize API status (locked/locked_by_me) over local selection
                ...
                <div className='flex flex-wrap justify-center gap-4 md:gap-8 mt-10 pt-6 border-t border-white/10'>
                  ...
                  <div className='flex items-center gap-3 px-4 py-2 rounded-lg bg-white/5 hover:bg-white/10 transition-colors'>
                    ...
                    <span className='text-gray-300 text-sm font-medium'>Trống</span>
                  </div>
                  ...
                  <div className='flex items-center gap-3 px-4 py-2 rounded-lg bg-white/5 hover:bg-white/10 transition-colors'>
                    ...
                    <span className='text-gray-300 text-sm font-medium'>Đã chọn</span>
                  </div>
                  ...
                  <div className='flex items-center gap-3 px-4 py-2 rounded-lg bg-white/5 hover:bg-white/10 transition-colors'>
                    ...
                    <span className='text-gray-300 text-sm font-medium'>Đã bán</span>
                  </div>
                  ...
                  <div className='flex items-center gap-3 px-4 py-2 rounded-lg bg-white/5 hover:bg-white/10 transition-colors'>
                    ...
                    <span className='text-gray-300 text-sm font-medium'>Đang giữ</span>
                  </div>
                </div>
              </div>
            </div>
